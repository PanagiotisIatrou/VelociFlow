...
void Node::correct_velocity_u() {
    ...
    const double correction = m_dy * (pressure_correction_w - pressure_correction_e) / momentum_u_a_P;
...
void Node::correct_velocity_v() {
    ...
    const double correction = m_dx * (pressure_correction_s - pressure_correction_n) / momentum_v_a_P;
