...
Coefficients SimplePressureCorrectionDivergence::get_effects() const {
    ...
    const double momentum_x_a_P = m_node->get_equation_coefficient(EquationType::MomentumX, CoefficientType::Center);
    const double momentum_y_a_P = m_node->get_equation_coefficient(EquationType::MomentumY, CoefficientType::Center);

    // a_W
    if (face_w->get_face_type() != FaceType::Boundary) {
        const double momentum_x_a_W = m_node->get_neighbouring_node(Direction::West)->get_equation_coefficient(EquationType::MomentumX, CoefficientType::Center);
    ...
    // a_E
    if (face_e->get_face_type() != FaceType::Boundary) {
        const double momentum_x_a_E = m_node->get_neighbouring_node(Direction::East)->get_equation_coefficient(EquationType::MomentumX, CoefficientType::Center);
    ...
    // a_S
    if (face_s->get_face_type() != FaceType::Boundary) {
        const double momentum_y_a_S = m_node->get_neighbouring_node(Direction::South)->get_equation_coefficient(EquationType::MomentumY, CoefficientType::Center);
    ...
    // a_N
    if (face_n->get_face_type() != FaceType::Boundary) {
        const double momentum_y_a_N = m_node->get_neighbouring_node(Direction::North)->get_equation_coefficient(EquationType::MomentumY, CoefficientType::Center);
