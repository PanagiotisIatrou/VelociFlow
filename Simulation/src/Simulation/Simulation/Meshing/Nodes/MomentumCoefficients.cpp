...
std::array<double, 6> Node::get_diffusion_effects(const VelocityComponent velocity_component) const {
    double source = 0.0;
    double a_P = 0.0;
    double a_W = 0.0;
    double a_E = 0.0;
    ...
    if (face_w->get_face_type() != FaceType::Boundary) {
        const double extra_a_W = face_w->get_viscosity() * m_dt * m_dy / m_dx;
        a_W += extra_a_W;
        a_P += extra_a_W;
    } else {
        ...
        double extra_a_P = 2.0 * face_w->get_viscosity() * m_dt * m_dy / m_dx;
        a_P += extra_a_P;
    ...
    if (face_e->get_face_type() != FaceType::Boundary) {
        const double extra_a_E = face_e->get_viscosity() * m_dt * m_dy / m_dx;
        a_E += extra_a_E;
        a_P += extra_a_E;
    } else {
        ...
        double extra_a_P = 2.0 * face_e->get_viscosity() * m_dt * m_dy / m_dx;
        a_P += extra_a_P;
    ...
    if (face_s->get_face_type() != FaceType::Boundary) {
        const double extra_a_S = face_s->get_viscosity() * m_dt * m_dx / m_dy;
        a_S += extra_a_S;
        a_P += extra_a_S;
    } else {
        ...
        double extra_a_P = 2.0 * face_s->get_viscosity() * m_dt * m_dx / m_dy;
        a_P += extra_a_P;
    ...
    if (face_n->get_face_type() != FaceType::Boundary) {
        const double extra_a_N = face_n->get_viscosity() * m_dt * m_dx / m_dy;
        a_N += extra_a_N;
        a_P += extra_a_N;
    } else {
        ...
        double extra_a_P = 2.0 * face_n->get_viscosity() * m_dt * m_dx / m_dy;
        a_P += extra_a_P;
...
std::array<double, 6> Node::get_convection_effects(const VelocityComponent velocity_component) const {
    double source = 0.0;
    double a_P = 0.0;
    double a_W = 0.0;
    double a_E = 0.0;
...
std::array<double, 6> Node::get_pressure_effects(const VelocityComponent velocity_component) const {
    double source = 0.0;
    double a_P = 0.0;
    double a_W = 0.0;
    double a_E = 0.0;
    ...
    if (velocity_component == VelocityComponent::U) {
        ...
        source += m_dt * m_dy * (face_w->get_pressure() - face_e->get_pressure());
    } else {
        Face *face_s = get_neighbouring_face(Direction::South);
        Face *face_n = get_neighbouring_face(Direction::North);

        source += m_dt * m_dx * (face_s->get_pressure() - face_n->get_pressure());
    }

    return {a_P, source, a_W, a_E, a_S, a_N};
}

std::array<double, 6> Node::get_time_effects(const VelocityComponent velocity_component) const {
    double source = 0.0;
    double a_P = 0.0;
    double a_W = 0.0;
    double a_E = 0.0;
    ...
    double extra = m_density * m_dx * m_dy;

    a_P += extra;

    if (velocity_component == VelocityComponent::U) {
        extra *= get_previous_timestep_velocity_u();
    } else {
        extra *= get_previous_timestep_velocity_v();
    }
    source += extra;
...
void Node::calculate_momentum_coefficients(const VelocityComponent velocity_component, const SimulationType simulation_type) {
    // Diffusion
    ...
    double a_P = diffusion_coefficients[0] + convection_coefficients[0] + pressure_coefficients[0] + time_coefficients[0];
    double source = diffusion_coefficients[1] + convection_coefficients[1] + pressure_coefficients[1] + time_coefficients[1];
    const double a_W = diffusion_coefficients[2] + convection_coefficients[2] + pressure_coefficients[2] + time_coefficients[2];
    const double a_E = diffusion_coefficients[3] + convection_coefficients[3] + pressure_coefficients[3] + time_coefficients[3];
    const double a_S = diffusion_coefficients[4] + convection_coefficients[4] + pressure_coefficients[4] + time_coefficients[4];
    const double a_N = diffusion_coefficients[5] + convection_coefficients[5] + pressure_coefficients[5] + time_coefficients[5];

    if (velocity_component == VelocityComponent::U) {
        // Under-relaxation
        a_P /= velocity_u_relaxation;
        source += (1 - velocity_u_relaxation) * a_P * get_velocity_u();

        m_momentum_u_coefficients = {a_P, source, a_W, a_E, a_S, a_N};
    } else {
        // Under-relaxation
        a_P /= velocity_v_relaxation;
        source += (1 - velocity_v_relaxation) * a_P * get_velocity_v();
