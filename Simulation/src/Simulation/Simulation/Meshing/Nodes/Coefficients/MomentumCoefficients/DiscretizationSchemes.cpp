...
SchemeCoefficients DiscretizationSchemes::convection(const Direction direction, const Scheme scheme,
    ...
    const double area = direction == Direction::WestWest || direction == Direction::West || direction == Direction::East
                        || direction == Direction::EastEast
                            ? m_node->m_dy
                            : m_node->m_dx;
    ...
    if (face->get_face_type() != FaceType::Boundary) {
        switch (scheme) {
            ...
            case Scheme::Hybrid: {
                const double length = direction == Direction::WestWest || direction == Direction::West
                                      || direction == Direction::East || direction == Direction::EastEast
                                          ? m_node->m_dx
                                          : m_node->m_dy;
...
SchemeCoefficients DiscretizationSchemes::hybrid(const double length, const double velocity, const double viscosity,
                                                 const double density, const double flow_rate) {
