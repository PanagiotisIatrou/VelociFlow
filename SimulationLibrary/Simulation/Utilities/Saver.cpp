...
void Saver::write_normalization_values(const EquationType equation_type, Equation *equation) const {
    ...
    fprintf(m_file, "%s,%f", equation_type_to_str(equation_type).c_str(), equation->get_imbalance_normalization_factor());

    // Extra normalization value only for the pressure correction equation
    // (mass imbalance)
    if (equation_type == EquationType::PressureCorrection) {
        fprintf(m_file, ",%f", static_cast<PressureCorrection *>(equation)->get_mass_imbalance_normalization_factor());
    }
