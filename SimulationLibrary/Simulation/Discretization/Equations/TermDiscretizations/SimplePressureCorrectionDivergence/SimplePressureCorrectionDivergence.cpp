...
Coefficients SimplePressureCorrectionDivergence::get_effects() const {
    ...
    for (int dir = 0; dir < direction_near_end; dir++) {
        ...
        direction_coefficients.source += sign * velocity_face * area;

        // Add the direction effects
        ...
        if (face_direction->get_face_type() != FaceType::Boundary) {
            ...
            const double extra_a_W =
                0.5 * (1 / momentum_a_P + 1 / momentum_a_direction) * m_node->get_dt() * area * area;
            ...
            const double extra = 0.5 * m_node->get_dt() * area * area / momentum_a_P;
