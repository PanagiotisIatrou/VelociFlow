...
Coefficients Pressure::get_effects() const {
    ...
    if (m_component == Component::X) {
        ...
        coefficients.source += m_node->get_dt() * m_node->get_dy() *
                               (face_w->get_field_value(Field::Pressure) - face_e->get_field_value(Field::Pressure));
    } else {
        ...
        coefficients.source += m_node->get_dt() * m_node->get_dx() *
                               (face_s->get_field_value(Field::Pressure) - face_n->get_field_value(Field::Pressure));
    }
