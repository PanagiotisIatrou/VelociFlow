...
MomentumXCoefficients::MomentumXCoefficients(Node *node, const Field variable_field,
    ...
    std::unique_ptr<Diffusion> diffusion_equation = std::make_unique<Diffusion>(
        node, Field::VelocityX, DiffusionSchemes::CentralDifferencing);
    ...
    std::unique_ptr<Convection> convection_equation = std::make_unique<Convection>(
        node, Field::VelocityX, ConvectionSchemes::QuickHayase, true);
